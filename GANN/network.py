...
class Controller:
    ...
    def setup(self):
        ...
        self.weights2 = self.individual[16:32].reshape((4,4))
        self.bias2 = self.individual[32:36].reshape((1,4))
        self.weights3 = self.individual[36:48].reshape((4,3))
    ...
    def forward(self, inputs):
        ...
        output = np.matmul(layer2, self.weights3)
        output = 20*np.pi/180*np.tanh(0.01*output)
        
        return output[0]
    
    def save(self, path):
        sio.savemat(path, mdict={"weights1": self.weights1, "bias1": self.bias1, "weights2": self.weights2, "bias2": self.bias2, "weights3": self.weights3})
